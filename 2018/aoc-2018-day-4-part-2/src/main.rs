...
fn main () -> Result <(), Box <dyn Error>> {
	...
	let mut guard_sleep_mins: HashMap <(u32, u32), u32> = HashMap::new ();
	...
	for line in input_lines.iter ().cloned () {
		...
		if text.starts_with ("Guard #") {
			...
			while cur_time < time {
				(* guard_sleep_mins.entry (
					(guard_id.unwrap (), cur_time.time ().minute ()),
				).or_insert (0)) += 1;
	...
	let ((sleepiest_id, sleepiest_min), _) = guard_sleep_mins.iter ().max_by_key (
		|(_, num)| * num,
	).unwrap ();
