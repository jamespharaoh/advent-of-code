...
fn main () {
	...
	for x in 0 .. input_values.len () - 1 {
		for y in x + 1 .. input_values.len () {
			if input_values [x] + input_values [y] == 2020 {
				println! ("First input value: {}", input_values [x]);
				println! ("Second input value: {}", input_values [y]);
				println! ("Product of both values: {}", input_values [x] * input_values [y]);
			}
		}
	}
}
