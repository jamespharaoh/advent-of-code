...
fn main () {
	...
	for swap_instr_idx in 0 .. instrs.len () {
		let mut instrs = instrs.clone ();
		instrs [swap_instr_idx].op = match instrs [swap_instr_idx].op {
			Op::Acc => continue,
			Op::Jmp => Op::Nop,
			Op::Nop => Op::Jmp,
		};
		...
		loop {
			if instr_idx == instrs.len () {
				println! ("Programme terminated with accumulator value: {}", acc);
				return;
			}
	...
	println! ("Unable to find terminating programme after swapping all jmp/nop instructinos");
}
