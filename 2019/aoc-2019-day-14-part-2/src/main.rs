...
fn main () {
	...
	let num_fuel = calculate_fuel (& input_lines);
	println! ("Total FUEL that we can make with 1 trillion ORE: {}", num_fuel);
}

fn calculate_fuel (input_lines: & Vec <String>) -> u64 {
	let mut fuel: u64 = 0;
	let mut step: u64 = 1_000_000_000;
	loop {
		if calculate_ore (input_lines, fuel + step) > 1_000_000_000_000 {
			if step == 1 { break }
			step /= 10;
			continue;
		}
...
fn calculate_ore (input_lines: & Vec <String>, num_fuel: u64) -> u64 {
	...
	quantities.insert ("FUEL".to_string (), num_fuel);
	for output_chemical in order {
		let reaction = & reactions [& output_chemical];
		let needed_quantity = quantities [& output_chemical];
		let reaction_times = (needed_quantity + reaction.output.quantity - 1) / reaction.output.quantity;
...
#[ test ]
fn test_0 () {
	assert_eq! (82892753, calculate_fuel (& vec! [
		"157 ORE => 5 NZVS",
		"165 ORE => 6 DCFZ",
...
#[ test ]
fn test_2 () {
	assert_eq! (5586022, calculate_fuel (& vec! [
...
#[ test ]
fn test_3 () {
	assert_eq! (460664, calculate_fuel (& vec! [
		"171 ORE => 8 CNZTR",
