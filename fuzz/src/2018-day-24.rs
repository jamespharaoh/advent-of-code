...
fuzz_target! (|input_str: & str| {
	...
	if let Ok (mut input) = Input::parse_from_lines (& input_vec) {
		input.params.max_rounds = cmp::min (input.params.max_rounds, 800);
		input.params.max_boost = cmp::min (input.params.max_boost, 40);
...
aoc_fuzz_mutator! {
	...
	transform add (1000) = |input, rng| {
		let mut group = Group {
			...
			weaknesses: ArrayVec::new (),
			immunities: ArrayVec::new (),
		...
		for attack_type in AttackType::VARIANTS {
			match rng.gen_range (0 .. 4) {
				0 => if ! group.weaknesses.is_full () { group.weaknesses.push (attack_type); },
				1 => if ! group.immunities.is_full () { group.immunities.push (attack_type); },
				_ => (),
			}
		}
		let groups =
	...
	transform modify (1000) = |input, rng| {
		...
		loop {
			match rng.gen_range (0 .. 9) {
				0 => group.num_units = rng.gen_range (1 ..= 9999),
				1 => group.hit_points = rng.gen_range (1 ..= 9999),
				2 => {
				...
				3 => {
					if group.weaknesses.is_full () { continue }
				...
				5 => {
					if group.immunities.is_full () { continue }
					let attack_type = AttackType::VARIANTS.choose (rng).copied ().unwrap ();
					if group.immunities.contains (& attack_type) { continue }
					group.immunities.push (attack_type);
				},
				6 => group.attack_damage = rng.gen_range (1 ..= 999),
				7 => group.attack_type = AttackType::VARIANTS.choose (rng).copied ().unwrap (),
				8 => group.initiative = rng.gen_range (1 ..= 99),
				_ => unreachable! (),
			}
			break;
		}
