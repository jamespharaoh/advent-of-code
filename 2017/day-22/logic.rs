...
fn expand (nodes: & mut Grid) {
	let incr_origin =
		cmp::min (nodes.raw_size () [0], nodes.raw_size () [1]) / 20 * 5 + 5;
	let incr_size = incr_origin * 2;
	let new_origin = [
		nodes.raw_origin () [0] + incr_origin.as_isize (),
		nodes.raw_origin () [1] + incr_origin.as_isize (),
	];
	let new_size = [
		nodes.raw_size () [0] + incr_size,
		nodes.raw_size () [1] + incr_size,
	];
	let new_data = iter::empty ()
		.chain (iter::repeat (Node::Clean).take (new_size [1] * incr_origin))
		.chain ((0 .. nodes.raw_size () [0]).flat_map (|row| iter::empty ()
			.chain (iter::repeat (Node::Clean).take (incr_origin))
			.chain (nodes.values ()
				.skip (nodes.raw_size () [1] * row)
				.take (nodes.raw_size () [1]))
			.chain (iter::repeat (Node::Clean).take (incr_origin))))
		.chain (iter::repeat (Node::Clean).take (new_size [1] * incr_origin))
		.collect ();
